...
var calendar = new Calendar(calendarEl, {
    ...
    // ]
    events: [
        // add event for each 4 hours for 10 seconds 
        {
            id: '0',
            title: 'Ligando pra não deixar ficar sem água no cano',
            description: 'horta',
            startTime: '09:01:00',
            endTime: '09:01:10',
            GPIO: '0',
...
// insert json in list_events div 
let list_events = document.querySelector('.list_events');
events.forEach(event => {
    let div = document.createElement('div');
    div.classList.add('event');
    div.innerHTML = `<h4>${event.title}</h4><p>${event.startStr}</p><p>${event.endStr}</p>`;
    list_events.appendChild(div);
});
